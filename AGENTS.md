...
- `bun run dev` - run the Astro dev server.
- `bun run build` - build static output and run the post-build minifier.
...
- [`public/boot.js`](public/boot.js): applies only first-paint theme mode plus main background/text colors before CSS loads.
...
- Keep first-paint theme values aligned between [`src/styles/tokens.css`](src/styles/tokens.css), [`src/app/core/settings.ts`](src/app/core/settings.ts), and [`public/boot.js`](public/boot.js).
...
- Keep [`public/boot.js`](public/boot.js) small, synchronous, and CSP-compatible; it should not become a general runtime loader.
